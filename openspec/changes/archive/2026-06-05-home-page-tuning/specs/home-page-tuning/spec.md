...
Define the accepted behavior for a narrow CSS-first tuning pass on the Home route `/` after shared `.btn` and focus primitives were adopted, preserving the current “AI Operations Control Room” direction without route, data, content, JSX structure, global primitive, Tailwind/shadcn, or `design-responsividad` changes.

## Requirements

### Requirement: Home CTA Ownership After Button Primitive Adoption

The Home page CTAs MUST use the shared `.btn` primitive layer for shared base shape behavior where that can be done without visible regression. Home-specific CTA typography, primary glow, secondary fill, hover/focus lift, visual weight, and hero emphasis MUST remain Home-owned and visually equivalent to the pre-tuning intent.
...
- GIVEN the Home tuning diff is inspected
- WHEN CSS focus rules are reviewed
- THEN no new global `a:focus-visible`, `button:focus-visible`, or universal focus selector MUST be added for this change
...
- WHEN responsive ownership is inspected
...
- AND Home tuning MUST NOT move or redesign page-level responsive architecture without explicit later approval
...
- GIVEN the Home tuning diff is reviewed
...
This change MUST remain a Home page tuning pass, not a redesign. It MUST NOT change routes, data fetching, content modules, metadata, control flow, JSX structure, global primitives, Tailwind/shadcn setup, broad responsive architecture, or files under `openspec/specs/design-responsividad/`. `src/app/page.tsx` MUST remain read-only by default unless a blocking accessibility issue is documented and explicitly approved in a later phase.

#### Scenario: Source scope stays Home CSS first

- GIVEN the implementation plan or diff is reviewed
- WHEN touched source files are inspected
- THEN `src/styles/home.css` SHOULD be the primary source target
- AND `src/app/page.tsx` MUST remain unchanged unless a blocking accessibility issue is documented and approved before implementation
...
- THEN shared `.btn` and focus primitives MUST NOT be redesigned as part of this change
...
The implementation MUST stay at or below a hard review budget of 300 changed lines. The workflow SHOULD stop before Apply if the forecast exceeds 150 source changed lines without explicit user approval. Verification MUST include lint, build, diff cleanliness, and Home route smoke checks for keyboard focus, dark mode, reduced motion, and mobile overflow.

#### Scenario: Review budget is enforced

- GIVEN the implementation forecast or completed diff is measured
- WHEN changed lines are counted
- THEN the total implementation review size MUST be at or below 300 changed lines unless explicit user approval is recorded
- AND the workflow SHOULD stop before Apply if source changes are forecast above 150 changed lines without explicit user approval
