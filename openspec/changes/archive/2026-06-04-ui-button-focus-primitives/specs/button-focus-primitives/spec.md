...
The system MUST provide a reusable, low-specificity `.btn` CSS class in `primitives.css` that owns only the shared base shape properties common across existing button-like elements: inline-flex display, centered content via `align-items: center`, a content gap, padding, border-radius, text-decoration removal, explicit cursor affordance, and a minimal transition baseline. `.btn` MUST NOT set background, color, border, box-shadow, font-size, or font-weight, because those properties vary intentionally by page and variant. `.btn` MUST NOT own page layout, responsive breakpoints, or responsive sizing retuning.
...
The system MAY provide conservative `.btn--primary`, `.btn--outline`, and `.btn--subtle` variant classes in `primitives.css` only when an existing pattern justifies the variant and when the variant can be added without breaking page-specific override chains. Each variant MUST set only the minimum properties needed to distinguish it (typically background, color, and optional border) and MUST NOT duplicate base shape properties already owned by `.btn`. Variants MUST NOT own hover, active, dark-mode, or responsive behavior.
...
#### Scenario: Subtle variant provides low-emphasis treatment

- GIVEN an element carries both `.btn` and `.btn--subtle`
- WHEN the element renders
- THEN `.btn--subtle` MUST supply a subdued visual-weight treatment appropriate for recovery or secondary navigation contexts
- AND page-specific opacity-hover or color-hover behavior MUST continue to apply from the page stylesheet

#### Scenario: Variant is skipped when no safe existing pattern exists

- GIVEN no existing page-owned element currently uses a matching variant pattern
...
The following interactive selectors MUST receive an explicit `:focus-visible` keyboard-focus treatment: `.form__button`, `.error-btn-primary`, `.error-btn-secondary`, `.not-found__link`, `.portfolio__link`, `.blog-article__back`, and `.article__link`. The `:focus-visible` treatment MUST apply a keyboard-only visible focus ring, MUST NOT trigger on mouse or touch interaction, and SHOULD use the established gold-ring style (`outline: 0.3rem solid var(--principal-color); outline-offset: 0.35rem; box-shadow: 0 0 0 0.6rem rgba(247, 185, 53, 0.18)`) unless an explicit component-specific reason requires a different treatment.
...
- THEN a visible focus ring MUST appear around that element
...
#### Scenario: Already-covered selectors retain their existing treatment

- GIVEN `.home-hero__cta-link` already has `:focus-visible` treatment in the existing grouped sidebar rule
- WHEN the implementation consolidates or normalizes focus-visible rules
- THEN `.home-hero__cta-link` MUST retain visible keyboard focus
...
The implementation MUST NOT add global `a:focus-visible`, `button:focus-visible`, `a:hover`, or `button:hover` rules that restyle all links or all buttons across every page. Focus-visible normalization MUST target only explicitly listed candidate selectors.

#### Scenario: Global selector rules are absent

- GIVEN the implementation diff is reviewed
- WHEN CSS files are inspected
- THEN no `a:focus-visible` or `button:focus-visible` rule that targets all links or all buttons MUST be present
...
- THEN the focus-visible behavior of that element MUST be whatever it was before this change
- AND the implementation MUST NOT accidentally alter its focus treatment

### Requirement: TSX Additive Class-Name Constraint

Any TSX file change in this implementation MUST be limited to adding one or more CSS class names to an existing element's `className` attribute. No content, data, control flow, component extraction, route logic, metadata, or structural markup changes are permitted in TSX files.
...
- WHEN `.btn` is added to the CTA element's class list
...
- WHEN focus-visible normalization is added for `.portfolio__link`
...
The `.btn` primitive and any focus-visible transitions it introduces MUST be compatible with the existing global reduced-motion baseline in `reset.css`. If `.btn` introduces CSS `transition` properties, they MUST be covered by the global baseline's `transition-duration: 0.01ms !important` under `prefers-reduced-motion: reduce`. The implementation MUST NOT introduce new `prefers-reduced-motion` blocks that duplicate or conflict with the global baseline unless a primitive introduces transform-based or opacity-based hover effects that need explicit suppression beyond duration minimization.
...
#### Scenario: No conflicting reduced-motion block

- GIVEN the implementation introduces transitions in `primitives.css`
- WHEN `primitives.css` is inspected
- THEN no standalone `@media (prefers-reduced-motion: reduce)` block MUST be present in `primitives.css` unless it suppresses transform or opacity effects not already covered by the duration-only global baseline

### Requirement: Non-Goal Enforcement

The implementation MUST NOT include any Tailwind or shadcn migration, page redesign, global button or link restyle, hover normalization, broad token migration (spacing, shadow, radius, border-width, color tokens), route/data/content changes, responsive retuning, or `openspec/specs/design-responsividad/` changes.
...
- AND no `src/data/` files MUST be modified
...
- GIVEN the implementation forecast is computed
- WHEN the forecast exceeds 300 changed lines
- THEN the apply phase MUST pause
- AND a delivery decision MUST be requested before continuing
...
The implementation MUST pass `npm run lint` and `npm run build` with no new attributable errors or warnings. Manual verification MUST cover keyboard Tab/focus-visible behavior across all in-scope selectors on their containing routes, dark-mode smoke checks, reduced-motion smoke checks, and a diff audit confirming additive-only TSX changes and no non-goal scope creep.
