...
export default function ThemeToggle() {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect -- one-time sync of external theme state on mount
    setDark(saved);
    setMounted(true);
  }, []);
  ...
  return (
    <button
      ...
      aria-label={mounted ? (dark ? "Cambiar a modo claro" : "Cambiar a modo oscuro") : "Cambiar tema"}
    >
      {mounted ? (
        dark ? (
        ...
        /* Placeholder while hydrating — same dimensions, invisible */
